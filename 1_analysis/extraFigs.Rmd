...
all_ora_DTvPBS <- rbind(go[[1]],kegg[[1]], react[[1]])
all_ora_TregvDT <- rbind(go[[3]],kegg[[3]], react[[3]])


library(ComplexHeatmap)
ora_id <- c("GO:0140507", "GO:0042267", "GO:0001910", "GO:0046703", "GO:0001913","GO:0019835","mmu04612","R-MMU-1474228")
...
anno_col <-  brewer.pal(3,"Set1")%>% setNames(c("GO", "KEGG", "Reactome"))
anno_col_2 <-  groupColour %>% setNames(Comp)

library(ggplotify)

createHeat <- function(matrix, compIndex, col){
  anno <- data.frame("Database" = c(rep("GO",6), rep("KEGG", 1), "Reactome"),
                   "Constrast" = rep(Comp[compIndex], 8))
  breaks <-seq(-2,2, by = 1)
  ...
  p <- ComplexHeatmap::pheatmap(
    ...
    legend = T,
    breaks = breaks,
    heatmap_legend_param = list(title = "LogFC", legend_width = unit(3, "cm"), limit = c(-2,2)),
...
p1 <- createHeat(mat1 %>% as.matrix(), 1, col = F)
p2 <- createHeat(mat2 %>% as.matrix(), 3, col = T)
