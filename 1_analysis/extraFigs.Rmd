...
createHeat <- function(matrix, compIndex){
  ...
  p <- ComplexHeatmap::pheatmap(
    ...
    show_colnames = T,
    legend = T,
    heatmap_legend_param = list(title = "LogFC", legend_width = unit(3, "cm")),
    ...
    fontsize_col = 14,
    fontsize_number = 14,
    fontsize_row = 14,row_names_side = "left", row_dend_side = "right"
...
p1 <- createHeat(mat[1:8,] %>% as.matrix(), 1)
p2 <- createHeat(mat[9:16,] %>% as.matrix(), 3)

sampleHeatmap <- p1/ p2

ggsave(filename = "sampleHeat.svg",plot = sampleHeatmap,path = here::here("2_plots/"),width = 21.5,height = 20,units = "cm")
