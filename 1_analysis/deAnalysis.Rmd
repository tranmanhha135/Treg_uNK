...
colnames(contrast) <- c("DT vs PBS", "Treg vs PBS", "DT vs Treg")
...
### PCA
```{r voomPCA, fig.cap="PCA plots after voom quality weight transformations. Group-specific weights(left) and sample-specific weights (right)"}
# Function for performing pca and generating plots
voom_pca <- function(voom_trans){
  pca <- voom_trans$E %>% 
    t() %>% 
    prcomp()
...
voom_pca_plots <- lapply(1:2, function(y){
  voom_pca(voom_trans = get(paste0('voom',y)))
})
...
if (fc == "none") {
  ...
  lm_all <- lapply(1:ncol(lm), function(y){limma::topTable(lm, coef = y, number = Inf, adjust.method = "fdr")}) %>% setNames(.,comp)
  } else {
  lm_fit <- limma::lmFit(object = voom1, design = full_design) %>%
    limma::contrasts.fit(contrasts = contrast) %>% limma::treat(fc = as.numeric(fc))
  lm_all <- lapply(1:ncol(lm), function(y){limma::topTreat(fit = lm, coef = 1, number = Inf, adjust.method = "fdr")}) %>% setNames(.,comp)
}
...
# for(name in comp) {
  venn.diagram(x = list("DT vs PBS"   = lm_sig[[1]]$gene %>% as.character(), 
                        ...
                        "DT vs Treg"  = lm_sig[[3]]$gene %>% as.character()),
