...
ipa_dot_all$name <- ipa_dot_all$name %>% str_wrap(45)
combine_ipa <- ggplot(ipa_dot_all) +
    geom_point(aes(x = group, y = name, colour = zScore, size = ratio)) +
    ...
    labs(x = "", y = "", color = "Z- scores", size = "Ratio")+
    bossTheme(base_size = 14,legend = "right")
  
  
  if(savePlots == TRUE) {
    ggsave(filename = paste0("combine_ipa_dot.svg"), plot = combine_ipa, path = here::here("2_plots/3_FA/ipa/"),
...
anno$combinedType <- factor(anno$combinedType, levels = c("Cytokine", "Group", "Transmembrane Receptor", "Transcription Regulator"))
...
ggsave(filename = "upstream_hmap.svg", plot = hmap_combined, path = here::here("2_plots/3_FA/ipa/"),
...
ggsave(filename = "dnf_dot.svg", plot = combine_ipa_dnf, path = here::here("2_plots/3_FA/ipa/"),
       width = 21, height = 29.7, units = "cm")
