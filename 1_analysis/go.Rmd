...
```{r functionalHeat}
go_id <- c("GO:0042267", "GO:0002228", "GO:0045953", "GO:0002716", "GO:0046703", "GO:0046641", "GO:0140507")

res <- enrichGO_sig[[1]][enrichGO_sig[[1]]$ID %in% go_id,]
...
for (i in seq_along(pathways)) {
  ...
  print(gns)
  mat[i, which(colnames(mat) %in% gns)] <- lm_sig[[1]][ gns, "logFC"]
}

pheatmap(mat)

partial <- enrichGO_sig[[1]][, c("ID", "geneID")]
partial <- partial[go_id, "geneID"] %>% as.data.frame()
partial <- separate_rows(partial, ., sep = "/")
genes <- partial$. %>% as.factor() %>% levels()


logfc <- lm_sig[[1]][lm_sig[[1]]$gene %in% genes, c("gene", "gene_id", "entrezid", "logFC", "adj.P.Val")]

colnames(partial) <- "entrezid"
heatplot(enrichGO[[1]], showCategory=5,)
