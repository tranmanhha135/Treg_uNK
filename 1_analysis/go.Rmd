...
for (i in 1:length(fc)) {
  for (j in 1:length(fdr)) {
    ...
    enrichGO_sig[[name]]$Description <- enrichGO_sig[[name]]$Description %>% str_to_sentence()
    
    # at the beginnning of a word (after 35 characters), add a newline. shorten the y axis for dot plot
    enrichGO_sig[[name]]$Description <- sub(pattern = "(.{1,45})(?:$| )",
                                         replacement = "\\1\n",
                                         x = enrichGO_sig[[name]]$Description)

    # # remove the additional newline at the end of the string
    enrichGO_sig[[name]]$Description <- sub(pattern = "\n$",
                                         replacement = "",
                                         x = enrichGO_sig[[name]]$Description)
...
for(name in names(enrichGO)){
  ...
  reducedTerms[[name]]$parentTerm <- reducedTerms[[name]]$parentTerm %>% str_to_sentence()
  reducedTerms[[name]]$parentTerm <- sub(pattern = "(.{1,35})(?:$| )", replacement = "\\1\n", x = reducedTerms[[name]]$parentTerm)
  reducedTerms[[name]]$parentTerm <- sub(pattern = "\n$", replacement = "", x = reducedTerms[[name]]$parentTerm)
  
  reducedTerms[[name]]$term <- reducedTerms[[name]]$term %>% str_to_sentence()
  reducedTerms[[name]]$term <- sub(pattern = "(.{1,35})(?:$| )", replacement = "\\1\n", x = reducedTerms[[name]]$term)
  reducedTerms[[name]]$term <- sub(pattern = "\n$", replacement = "", x = reducedTerms[[name]]$term)
...
Again, for data exploratory purposes, the following visualisations are GO enrichment analysis performed with set of DE genes significantly below **FDR \< 0.1 or \< 0.05** with and without FC threshold (TREAT). IMPORTANTLY, these GO terms are all significantly enriched (FDR <0.05)
...
#### Dot plot
```{r dot, fig.width=8, fig.height=10}
dot <- list()
tab <- list()
upset <- list()
...
for (i in 1:length(fc)) {
  for (j in 1:length(fdr)) {
    ...
    dot[[name]] <- ggplot(enrichGO_sig[[name]][1:25, ]) +
...
types <- c("Dot plot", "Table", "Upset plot", "Dendrogram", "Scatter", "Treemap")
...
for (i in 2:length(headers)) {
  ...
  cat(paste0("#### ",types[[6]]," \n"))
  # adjTreemap(x = reducedTerms[[i]])
  special_kexpand(wd = 9,ht = 9,cap = paste0("upset",i),res = "hide",echo = "FALSE")
  cat("\n\n")
